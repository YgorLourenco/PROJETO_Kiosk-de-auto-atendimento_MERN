// Gerenciamento de variaveis para o Context
// Ação para entrar na página de pedidos
export const ORDER_SET_TYPE = 'ORDER_SET_TYPE'
// Ação para abrir a lista das categorias na página de pedidos
export const CATEGORY_LIST_REQUEST = 'CATEGORY_LIST_REQUEST'
export const CATEGORY_LIST_SUCCESS = 'CATEGORY_LIST_SUCCESS'
export const CATEGORY_LIST_FAIL = 'CATEGORY_LIST_FAIL'
// Ação para abrir a lista de produtos na página de pedidos
export const PRODUCT_LIST_REQUEST = 'PRODUCT_LIST_REQUEST'
export const PRODUCT_LIST_SUCCESS = 'PRODUCT_LIST_SUCCESS'
export const PRODUCT_LIST_FAIL = 'PRODUCT_LIST_FAIL'
// Ação para remover e adicionar items no pedido
export const ORDER_ADD_ITEM = 'ORDER_ADD_ITEM'
export const ORDER_REMOVE_ITEM = 'ORDER_REMOVE_ITEM'
// Vai apagar todo o pedido
export const ORDER_CLEAR = 'ORDER_CLEAR'
// Ação para o tipo do pagamento para o pedido
export const ORDER_SET_PAYMENT_TYPE = 'ORDER_SET_PAYMENT_TYPE'
// Ação para criar uma requisição para o pedido
export const ORDER_CREATE_REQUEST = 'ORDER_CREATE_REQUEST'
export const ORDER_CREATE_SUCCESS = 'ORDER_CREATE_SUCCESS'
export const ORDER_CREATE_FAIL = 'ORDER_CREATE_FAIL'